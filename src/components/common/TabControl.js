    ...
    render() {
        ...
        return (
            <div className={p.className}>
                <ul className='TabBar'>
                    { items.map(item => { 
                        return <button key={item.id} className={'TabItem' + (this.currentItem === item ? ' Active' : '')} onClick={() => { 
                            this.setVisibleItem(item)
                        }}>{item[p.tabCaptionField]}</button>
                     }) }
                </ul>
                <div className='TabContent'>
                    { this.currentItem ? (cc && cc(this.currentItem)) : null }
                </div>
            </div>
        )
    }
