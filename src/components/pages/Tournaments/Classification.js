...
class Classification extends Component {
    ...
    render() {
        ...
        if (!stages || stages.length === 0) return <InfoBox><Loc>Classification.NoStages</Loc></InfoBox>;

        const defaultTabIndex = store.getActiveStageIndex();

        return (
            <TabControl defaultTab={defaultTabIndex} items={stages} contentCallback={(stage) => <StageClassification stage={stage} tournament={tournament} />} />
        )
    }
}
