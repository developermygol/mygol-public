    ...
    getPoints() {
        ...
        return (
            positions.map((pos, i) => {
                ...
                return (
                    <div key={i} className='TacticPlayer' style={{ position: 'absolute', bottom: (pos.x - 1) * 100 / 16 + '%', left: (pos.y - 1) * 100 / 10 + '%' }}>
                        {player ? getUploadsIcon(player.userData && player.userData.avatarImgUrl, player.id, 'user'): null }
                    </div>
                )
            })
        )
    ...
    render() {
        ...
        return (
            ...
            <div className='TacticContainer'>
                <div className='TacticField' style={{ backgroundImage: 'url(' + getBaseUrl(process.env.REACT_APP_STATIC_STATIC_URL) + '/tactics/' + image }}>
                    <div className='TacticPositions'>
                        {this.getPoints()}
                    </div>
                </div>
            </div>
