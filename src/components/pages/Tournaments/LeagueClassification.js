...
class LeagueClassification extends Component {
    ...
    render() {
        ...
        if (!data) return <InfoBox><Loc>Classification.NoGroups</Loc></InfoBox>;

        const normalTeams = normalize(teams);

        if (data.length === 1) {
            return <LeagueGroupClassification classification={data[0].grouped} normalTeams={normalTeams} tournament={tnmt} />
        }
        ...
        return data.map(group => {
            return (
                <div key={group.id} className=''>
                    <h4>{group.name}</h4>
                    <LeagueGroupClassification classification={group.grouped} normalTeams={normalTeams} tournament={tnmt} />
                </div>
            )
        })
    }
