...
class SanctionList extends Component {
  ...
  getSanctionLink = row => {
    ...
    return (
      <Link to={'/tournaments/' + idTournament + '/sanctions/' + row.id}>
  ...
  playerColumns = [
    { id: 'c1', label: Localize('Sanctions.Date'), handler: this.getSanctionLink, headerClassName: 'SkewM' },
    {
      ...
      handler: r => this.getPlayer(r.player),
      className: 'Text',
    ...
    },
    {
      id: 'c5',
      label: Localize('Team'),
      handler: r => this.getTeam(r.idTeam),
      className: 'Text',
      headerClassName: 'SkewM',
    },
    {
      id: 'c6',
      label: Localize('Sanctions.NumMatches'),
      handler: r => r.numMatches,
      className: 'Integer',
      headerClassName: 'Center SkewM',
    },
    {
      id: 'c7',
      label: Localize('Sanctions.Status'),
      handler: textLookup('SanctionStatus', 'status'),
      headerClassName: 'SkewM',
    },
  ];
  ...
  teamColumns = [
    ...
    {
      id: 'c2',
      label: Localize('Sanctions.Title'),
      fieldValue: 'title',
      className: 'Text',
      headerClassName: 'SkewM',
    },
      ...
      <Spinner loading={p.store.sanctions.loading}>
        <Fragment>
          <div className="Section">
            {ss.hasTeams && (
              <h4>
                <Loc>Sanctions.Players</Loc>
              </h4>
            )}
            <div className="Content">
              <DataTable columns={this.playerColumns} data={ss.playerSanctions} isDataNormalized={false} />
            </div>
          </div>
          {ss.hasTeams && (
            <div className="Section">
              <h4>
                <Loc>Sanctions.Teams</Loc>
              </h4>
              <div className="Content">
                <DataTable columns={this.teamColumns} data={ss.teamSanctions} isDataNormalized={false} />
              </div>
            </div>
          )}
        </Fragment>
