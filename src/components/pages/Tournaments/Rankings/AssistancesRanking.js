...
class AssistancesRanking extends Component {
  ...
  setVisibleRanking = type => {
    ...
    requestAsync(
      ...
      axios.get,
      null,
      '/tournaments/' + idTournament + '/ranking/assistances/' + type + limit
    ).then(
      action(res => {
        ...
        {isSpecial && firstPlayer && (
          <div className="LargeRankingCell">
            <div className="RankingTop">
              <div className="Award Award_MaxAssistances">
                <span className="Value">Asistencias: {first.assistances}</span>
              </div>
            </div>
