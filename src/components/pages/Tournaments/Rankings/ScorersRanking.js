...
class ScorersRanking extends Component {
  ...
  setVisibleRanking = async type => {
    ...
    requestAsync(
      ...
      axios.get,
      null,
      '/tournaments/' + idTournament + '/ranking/scorers/' + type + limit
    ).then(
      action(res => {
        ...
        {isSpecial && firstPlayer && (
          <div className="LargeRankingCell">
            <div className="RankingTop">
              <div className="Award Award_MaxScorer">
                <span className="Value">Goles: {first.points}</span>
              </div>
            </div>
