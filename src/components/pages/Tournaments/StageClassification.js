...
class StageClassification extends Component {
  ...
  update = () => {
    requestAsync(this, axios.get, null, '/tournaments/stageclassification/' + this.props.stage.id).then(
      res => (this.loadedStage = res)
    );
  };
      ...
      if (ls.leagueClassification)
        return (
          <LeagueClassification
            stage={stage}
            classification={ls.leagueClassification}
            teams={teams}
            tournament={t}
          />
        );
        ...
        return (
          <KnockoutClassification classification={ls.knockoutClassification} teams={teams} tournament={t} />
        );
      else
        return (
          <InfoBox>
            {stage.name}
            <Loc>Classification.NoData</Loc>
          </InfoBox>
        );
    }
